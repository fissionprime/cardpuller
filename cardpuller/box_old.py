        ...
        for i, remaining in enumerate(self.boxstats[1]):
            while remaining:
                for index in range(0, self.boxstats[0][i]):
                    if not remaining:
                        break
                    self.list.append(card(self.rarities[i], index, (index < self.extras[i])))
...
for card in targets:
    ...
    if card.get('extra', False):
        maxgoal.append(math.ceil(newbox.boxstats[1][newbox.rarities.index(card['rarity'])]
                                 / newbox.boxstats[0][newbox.rarities.index(card['rarity'])]))
    else:
        maxgoal.append(math.floor(newbox.boxstats[1][newbox.rarities.index(card['rarity'])]
                                  / newbox.boxstats[0][newbox.rarities.index(card['rarity'])]))
othercards = (newbox.template['packs_total'] * 3) - sum(maxgoal)
...
for pack in range(1, newbox.template['packs_total'] + 1):
...
# theoreticalcdf = []
# theoreticalpmf = []
# desired_rarity = newbox.rarities.index(desired_rarity)
# numcards = newbox.boxstats[1][desired_rarity] / newbox.boxstats[0][desired_rarity]
# if desired_extra:
...
#
# desired_rarity = newbox.rarities[desired_rarity]
